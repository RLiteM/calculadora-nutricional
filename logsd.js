...
function enviar() {
  ...
  // ✅ TOMAMOS EL VALOR MOSTRADO EN PANTALLA
  const kilosText = document.getElementById("kilos").innerText;
  const kilosMatch = kilosText.match(/([\d.]+)/);
  let pesoMostrado = 0;

  if (kilosMatch) {
    const kilosValue = parseFloat(kilosMatch[1]);
    // ✅ TRUNCAMOS A UN DECIMAL SIN REDONDEO
    pesoMostrado = Math.trunc(kilosValue * 10) / 10;
  }

  const fechaNac = document.getElementById("fecha_nac").value;
  const fechaEval = document.getElementById("fecha_eval").value;

  if (pesoMostrado <= 0 || isNaN(talla) || !fechaNac || !fechaEval || !sexoSeleccionado) {
  ...
  const payload = {
    peso: pesoMostrado,  // ✅ EL VALOR VISIBLE EN PANTALLA, TRUNCADO
    talla: talla,
